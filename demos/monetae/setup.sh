#!/bin/bash
# Setup script for Monetae demo ledger
#
# Prerequisites:
...
LEDGER_NAME="${LEDGER_NAME:-monetae-demo}"

echo "Setting up Monetae demo ledger..."
...
RESULT=$(curl -s -X POST "$API_URL/api/ledger/v2/$LEDGER_NAME" \
  ...
  -d "{\"bucket\": \"$LEDGER_NAME\", \"metadata\": {\"demo\": \"monetae\"}}")
...
echo "http://localhost:3200/yhkjzhkjctlk/txpg/ledgers/$LEDGER_NAME?region=local-dev&demo=true"
echo ""
echo "Make sure the console has the monetae demo config in useDemoMode.ts"
